    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (lockScreenViews == null) {
            ...
            for (int i = 0; i < itemCount * itemCount; i++) {
                lockScreenViews[i] = new LockScreenView(getContext(), normalColor, smallRadius, bigRadius, rightColor, wrongColor);
                lockScreenViews[i].setId(i + 1);
                LayoutParams params = new LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);

                // 不能通过lockScreenViews[i].getMeasuredWidth()来获取宽高
                // 因为这时它的宽高还没有测量出来
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (view != null) {
                    ...
                    if (!mCurrentViews.contains(id)) {
                        ...
                        view.setCurrentState(LockScreenView.State.STATE_CHOOSED);
